...
@NoArgsConstructor
@Data
@ToString(exclude = {"files"}) //ToString에서 특정 필드 제외하기 files는 fetch타입이 LAZY이기 때문에 로딩되지 않음. -> Test 시 오류 발생
public class PDSBoardEntity {
	...
	//join칼럼이 생략되면 중간테이블이 생성된다. PDSBoard키, PDSFileEntity키를 칼럼으로 갖는 중간테이블만들어짐.
